...
void searchstring::print(int count, std::vector<int> indexes, double runtime) {
    ...
    if (count == 0) {
        ...
        // Print runtime
        std::cout << "\r\nThis algorithm took " << runtime << " milliseconds to run." << std::endl;
    ...
    if (m_NIdx.size() > 0 && m_RKIdx.size() > 0 && m_BMIdx.size() > 0) {
        ...
        if (yesNo == "yes") {
            ...
            for (unsigned int i = 0; i < m_RKIdx.size(); i++) {
                ...
                // Insert the entire "replacement" vector at index .begin() + occurrence
                m_text.insert(m_text.begin() + m_RKIdx[i], replacement.begin(), replacement.end());
                
                // Recalculate index values due to vector "growth"
                if (i < m_RKIdx.size()) {

                    // New index is now (previous) + (size of inserted phrase)
                    for (unsigned int j = 1 + count; j < m_RKIdx.size(); j++) {
...
// CHANGE NEWLINE INDEXES FUNCTION
void searchstring::changeNewline(int rs) {

    // Set the difference between search phrase and replacement phrase
    int newlineShift = rs - m_ss;

    // Change newline indexes if a single line
    for (unsigned int i = 0; i < m_RKIdx.size(); i++) {
    ...
    // Change newline indexes if more than a single line
    //if (m_writeIdx.size() != 1) {
        /*for (int i = 0; i < m_RKIdx.size(); i++) {
            for (unsigned int j = 0; j < m_writeIdx.size(); j++) {
                m_writeIdx[j] += newlineShift;
            }
        }*/

    // IMPLEMENTATION IS WIP, FORMATTING ISSUES ARE OCCURRING WHEN THE TEXT FILE CONTAINS MORE THAN ONE NEWLINE
    // (STILL FINDS AND REPLACES THE SEARCH PHRASE CORRECTLY, JUST THE NEWLINE OUTPUTS IN WRITEFILE ARE INCORRECT
