...
std::vector<char> proj::getPhrase() {
    ...
    //use getline to the strPhrase from user so that it preserves spaces and such
    std::getline(std::cin >> std::ws, strPhrase); 

    //copies the string to the back of the char vector
...
// WRITE FILE FUNCTION
void proj::writeFile(std::vector<char>& text, std::string outputName, std::vector<int>& index, int phraseDif, std::vector<int> rowOcur) {

    for (int i = 0; i < rowOcur.size(); i++) {
        std::cout << rowOcur[i] << ' ';
    }
    ...
    for (int i = 0; i < index.size(); i++) { 
        ...
        std::cout << index[i] << ' ';
        //phraseDif needs to be multipled by the amount of times the phrase is replaced on each line
        //resettting the amount of characters per row to the original + the difference int length between the searched phrase and the new phrase
        //multiplied by the amount of times the phrase is repeated on each row
        index[i] = index[i] + phraseDif * rowOcur[i]; 
...
std::vector<int> proj::rabinKarp(std::vector<char>& text, std::vector<char>& search, bool& found, std::vector<int>& rowOcur, std::vector<int> index) {
    ...
    //count number of matches on each row
    int rowTrack = 0;
...
void proj::replace(std::vector<char>& text, std::vector<int> foundIndex, std::vector<char> search, bool found, int& phraseDif) {
    char yesOrNo;

    //If the search phrase is found in the input file
    if (found == 1) {
        ...
        if (yesOrNo == 'y') {
            ...
            //difference in size between the phrase being replaced and the replacement phrase
            phraseDif = nextPhrase.size() - search.size();

            std::cout << std::endl;
            for (int i = 0; i < foundIndex.size(); i++) {

                //index of the first instance of the search phrase plus the difference between the search string and the new phjase * the amount of times we want to replace it
                int curIndex = foundIndex[i] + i * phraseDif; 
